    ...
    public partial class FormTractorConfig : Form
    {
        ...
        private void DrawTractor()
        {
            if (tractor != null)
            {
                ...
                tractor.SetPosition(100, 50, pictureBoxTractor.Width, pictureBoxTractor.Height);
        ...
        private void labelTractorWithLadle_MouseDown(object sender, MouseEventArgs e)
        {
            labelTractorWithLadle.DoDragDrop(labelTractorWithLadle.Text, DragDropEffects.Move | DragDropEffects.Copy);
        }
        ...
        private void panelTractor_DragDrop(object sender, DragEventArgs e)
        {
            switch (e.Data.GetData(DataFormats.Text).ToString())
            {
                case "Обычный Трактор":
                    ...
                    break;
                case "Трактор с ковшом":
                    tractor = new TractorWithLadle(100, 100, Color.Black, Color.Red, Color.Blue, true);
                    break;
            }
            DrawTractor();
        }
        ...
        private void labelDopColor_DragDrop(object sender, DragEventArgs e)
        {
            if (tractor != null)
            {
                if (tractor is TractorWithLadle)
                {
                    (tractor as TractorWithLadle).SetDopColor((Color)e.Data.GetData(typeof(Color)));
                    DrawTractor();
                }
            }
        }
